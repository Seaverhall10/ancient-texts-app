"""
era_85_zephaniah.py -- Zephaniah 1-3: Cosmic Judgment, Remnant, and YHWH Singing

The most comprehensive Day of YHWH vision: creation undone, nations judged, and
then -- astonishingly -- YHWH rejoicing over his people with singing.
"""

CHAPTERS = [
    {
        "id": "zephaniah-1",
        "ref": "Zephaniah 1",
        "chapter_num": 1,
        "title": "The Day of YHWH -- Cosmic Un-Creation",
        "era": "zephaniah",
        "type": "chapter",

        "synopsis": "Zephaniah 1 opens with the most sweeping judgment oracle in the OT: 'I will utterly "
                    "sweep away everything from the face of the earth... man and beast... birds and fish' "
                    "(1:2-3). This reverses Genesis 1: the created order is systematically undone. The charges "
                    "against Judah: Baal worship, astral cults ('those who bow down on the roofs to the host "
                    "of the heavens'), Milcom devotion, and spiritual apathy ('those who say in their hearts, "
                    "YHWH will not do good, nor will he do ill,' 1:12). The Day of YHWH description (1:14-18) "
                    "is the most terrifying in the prophets: 'A day of wrath... distress and anguish... "
                    "ruin and devastation... darkness and gloom... clouds and thick darkness... trumpet blast "
                    "and battle cry.' Neither silver nor gold can deliver in the day of YHWH's wrath.",

        "key_verse": {
            "ref": "Zephaniah 1:14-15",
            "text": "The great day of the LORD is near, near and hastening fast; the sound of the day of the LORD is bitter; the mighty man cries aloud there. A day of wrath is that day, a day of distress and anguish, a day of ruin and devastation, a day of darkness and gloom, a day of clouds and thick darkness.",
            "translation": "ESV"
        },

        "hebrew_terms": [
            "asoph aseph (I will utterly sweep away -- intensive form emphasizing totality)",
            "tseva ha-shamayim (host of the heavens -- the celestial beings worshiped in astral cults)",
            "yom evrah (day of wrath -- the phrase that inspired the medieval Dies Irae hymn)"
        ],

        "ane_backdrop": "Astral worship -- bowing to the sun, moon, stars, and planets on rooftops -- was "
                        "widespread in the ancient Near East and had infiltrated Judah during the Assyrian "
                        "period. The Mesopotamian astral pantheon (Shamash, Sin, Ishtar) was the theological "
                        "background for this syncretism.",

        "second_temple": [
            {
                "source": "Revelation 6:15-17",
                "summary": "The sixth seal: 'The great day of their wrath has come, and who can stand?'",
                "relevance": "Revelation's Day of wrath draws on Zephaniah's comprehensive judgment imagery."
            }
        ],

        "cross_refs": [
            {"ref": "Genesis 1:20-27", "note": "Creation of birds, fish, beasts, man -- Zephaniah 1:2-3 reverses this order", "type": "ot"},
            {"ref": "Deuteronomy 4:19", "note": "Warning against worshiping the host of heaven -- what Zephaniah condemns", "type": "ot"},
            {"ref": "Joel 2:1-2", "note": "Joel's Day of YHWH -- darkness and gloom; Zephaniah expands the vision", "type": "ot"},
            {"ref": "Revelation 6:15-17", "note": "'The great day of their wrath has come'", "type": "nt"},
            {"ref": "2 Peter 3:10", "note": "'The day of the Lord will come like a thief' -- the cosmic dissolution", "type": "nt"}
        ],

        "divine_council_note": "The cosmic un-creation of Zephaniah 1:2-3 represents the divine council's "
                               "ultimate judgment: the sovereign undoes his own creation. The condemnation of "
                               "astral worship (1:5) is a divine council text: Judah worships the 'host of "
                               "heaven' -- the celestial divine council members (Deut 4:19-20; 17:3) -- instead "
                               "of YHWH who commands them. The Day of YHWH is when the Most High judges both "
                               "the rebellious nations and the rebellious members of his own council.",

        "sections": [
            {
                "heading": "The Cosmic Sweep (1:1-6)",
                "body": "YHWH will sweep away everything: man, beast, birds, fish. Judah is specifically "
                        "targeted: the remnants of Baal, idolatrous priests, astral worshipers, Milcom "
                        "devotees, and the spiritually apathetic who think YHWH is inactive."
            },
            {
                "heading": "The Sacrifice and the Search (1:7-13)",
                "body": "'Be silent before the Lord YHWH! For the day of YHWH is near; YHWH has prepared "
                        "a sacrifice and consecrated his guests' (1:7). The 'sacrifice' is Judah itself. "
                        "YHWH will search Jerusalem with lamps, finding those who are 'thickening on their "
                        "dregs' -- settled, complacent, assuming YHWH does nothing."
            },
            {
                "heading": "The Great Day Described (1:14-18)",
                "body": "The most vivid Day of YHWH description in the OT: seven pairs of disaster terms "
                        "(wrath/distress, anguish/ruin, devastation/darkness, gloom/clouds, thick darkness/ "
                        "trumpet, battle cry). Neither silver nor gold delivers. 'In the fire of his jealousy, "
                        "all the earth shall be consumed.'"
            }
        ]
    },

    {
        "id": "zephaniah-2",
        "ref": "Zephaniah 2",
        "chapter_num": 2,
        "title": "Seek YHWH -- Oracles Against the Nations",
        "era": "zephaniah",
        "type": "chapter",

        "synopsis": "Between cosmic judgment (ch. 1) and Jerusalem's indictment (ch. 3), Zephaniah issues "
                    "a call to seek YHWH and then pronounces oracles against the surrounding nations. 'Seek "
                    "YHWH, all you humble of the land, who do his just commands; seek righteousness, seek "
                    "humility; perhaps you may be hidden on the day of the anger of YHWH' (2:3). Then: "
                    "Philistia will be depopulated (2:4-7), Moab and Ammon will become like Sodom and "
                    "Gomorrah (2:8-11), Ethiopia will be slain (2:12), and Nineveh will become a desolation "
                    "(2:13-15). The phrase 'YHWH will be awesome against them; for he will famish all the "
                    "gods of the earth' (2:11) is remarkable: YHWH will starve the rival divine beings by "
                    "eliminating their worshipers.",

        "key_verse": {
            "ref": "Zephaniah 2:3",
            "text": "Seek the LORD, all you humble of the land, who do his just commands; seek righteousness, seek humility; perhaps you may be hidden on the day of the anger of the LORD.",
            "translation": "ESV"
        },

        "hebrew_terms": [
            "anavim (humble/poor of the land -- the remnant who seek YHWH)",
            "razah (to famish/make lean -- YHWH will 'famish' the gods of the earth; starve them of worship)",
            "sater (to hide/shelter -- 'perhaps you may be hidden'; the prophet's name means 'YHWH hides')"
        ],

        "ane_backdrop": "The oracles against the nations cover the four compass directions: Philistia (west), "
                        "Moab/Ammon (east), Ethiopia (south), Assyria (north). The comprehensive geography "
                        "emphasizes the universality of judgment.",

        "second_temple": [
            {
                "source": "Isaiah 2:18-19",
                "summary": "'The idols shall utterly pass away. And people shall enter the caves of the rocks... "
                           "before the terror of YHWH.'",
                "relevance": "Isaiah's parallel vision of idol-elimination connects to Zephaniah's 'famishing' "
                             "of the gods."
            }
        ],

        "cross_refs": [
            {"ref": "Genesis 19:24-25", "note": "Sodom and Gomorrah -- Moab and Ammon will suffer the same fate (2:9)", "type": "ot"},
            {"ref": "Nahum 2-3", "note": "Nahum's parallel oracle against Nineveh", "type": "ot"},
            {"ref": "Isaiah 14:12-15", "note": "The fall of the arrogant -- Nineveh says 'I am, and there is no one else' (2:15)", "type": "ot"}
        ],

        "divine_council_note": "Zephaniah 2:11 -- 'YHWH will famish (razah) all the gods of the earth' -- is "
                               "a stunning divine council text. YHWH will 'starve' the gods by eliminating "
                               "their worship base. In the divine council framework, the allotted gods received "
                               "worship from their nations. When YHWH judges those nations, the gods are "
                               "deprived of their sustaining worship. This is a form of divine council "
                               "demotion: the rival elohim are rendered impotent by losing their worshipers.",

        "sections": [
            {
                "heading": "The Call to Seek YHWH (2:1-3)",
                "body": "Before the Day falls, seek YHWH, seek righteousness, seek humility. 'Perhaps' -- "
                        "the humility of the conditional; no guarantees, only hope through genuine seeking."
            },
            {
                "heading": "Oracles Against the Nations (2:4-15)",
                "body": "Philistia: depopulated, given to Judah's remnant. Moab/Ammon: like Sodom. "
                        "Ethiopia: slain. Nineveh: a desolation, pasture for flocks. YHWH famishes all "
                        "the gods of the earth. Every direction faces judgment."
            }
        ]
    },

    {
        "id": "zephaniah-3",
        "ref": "Zephaniah 3",
        "chapter_num": 3,
        "title": "Jerusalem Indicted, Then YHWH Sings Over His People",
        "era": "zephaniah",
        "type": "chapter",

        "synopsis": "Chapter 3 turns the lens on Jerusalem itself: 'Woe to her who is rebellious and "
                    "defiled, the oppressing city! She listens to no voice; she accepts no correction' (3:1-2). "
                    "Officials are roaring lions, judges are evening wolves, prophets are fickle, priests "
                    "profane the holy. 'YHWH within her is righteous; he does no injustice; every morning he "
                    "shows forth his justice' (3:5) -- the contrast between YHWH's character and Jerusalem's. "
                    "Then the pivot to restoration: YHWH will purify the nations, giving them 'pure speech' "
                    "to call on his name with one accord (3:9). The remnant of Israel will be 'a people "
                    "humble and lowly' who take refuge in YHWH (3:12). The climactic promise (3:17) is "
                    "breathtaking: 'YHWH your God is in your midst, a mighty one who will save; he will "
                    "rejoice over you with gladness; he will quiet you by his love; he will exult over you "
                    "with loud singing.'",

        "key_verse": {
            "ref": "Zephaniah 3:17",
            "text": "The LORD your God is in your midst, a mighty one who will save; he will rejoice over you with gladness; he will quiet you by his love; he will exult over you with loud singing.",
            "translation": "ESV"
        },

        "hebrew_terms": [
            "gibbor yoshia (mighty one who saves -- the divine warrior, now as savior-lover)",
            "yagil (he will rejoice -- YHWH's exultant joy over his restored people)",
            "yacharish be-ahavato (he will quiet you by his love -- YHWH's love silences fear and shame)",
            "yasis (he will exult with singing -- YHWH himself sings over his people)",
            "saphah berurah (pure speech -- the reversal of Babel's confused languages)"
        ],

        "ane_backdrop": "The image of a deity rejoicing and singing over a people is virtually unique in "
                        "ancient Near Eastern literature. The gods of Mesopotamia and Egypt are served by "
                        "humans; they do not sing over them with joy. Zephaniah's portrait of YHWH as the "
                        "joyful singer is theologically unprecedented.",

        "second_temple": [
            {
                "source": "Revelation 21:3-4",
                "summary": "'Behold, the dwelling place of God is with man. He will dwell with them... "
                           "He will wipe away every tear.'",
                "relevance": "The ultimate fulfillment of Zephaniah 3:17: God in the midst of his people, "
                             "quieting them by his love."
            }
        ],

        "cross_refs": [
            {"ref": "Genesis 11:1-9", "note": "Babel's confused languages -- Zephaniah 3:9 reverses this with 'pure speech'", "type": "ot"},
            {"ref": "Isaiah 12:6", "note": "'Shout and sing for joy... for great in your midst is the Holy One of Israel'", "type": "ot"},
            {"ref": "Isaiah 62:5", "note": "'As the bridegroom rejoices over the bride, so shall your God rejoice over you'", "type": "ot"},
            {"ref": "Acts 2:1-11", "note": "Pentecost -- the reversal of Babel that Zephaniah 3:9 anticipates", "type": "nt"},
            {"ref": "Revelation 21:3-4", "note": "God dwelling with his people -- the ultimate fulfillment", "type": "nt"}
        ],

        "divine_council_note": "Zephaniah 3:17 presents the divine council sovereign in his most intimate "
                               "posture: not enthroned in judgment but rejoicing, quieting, singing over his "
                               "people. The reversal from chapter 1's cosmic wrath to chapter 3's cosmic love "
                               "is extraordinary. The 'pure speech' of 3:9 reverses Babel (Gen 11): the "
                               "nations that were confused and allotted to the sons of God will be given one "
                               "language to worship YHWH together. This is the Deuteronomy 32 allotment "
                               "reversed: the scattered nations reunited in worship of the Most High.",

        "sections": [
            {
                "heading": "Jerusalem's Indictment (3:1-7)",
                "body": "The city's leaders are predators: officials like lions, judges like wolves, prophets "
                        "treacherous, priests profaning what is holy. YHWH is righteous in her midst, but "
                        "the unjust know no shame."
            },
            {
                "heading": "Pure Speech and the Humble Remnant (3:8-13)",
                "body": "YHWH will purify the nations, giving them 'pure speech' to serve him with one "
                        "accord. The remnant of Israel will be humble and lowly, taking refuge in YHWH's "
                        "name. No more pride, no more arrogance on YHWH's holy mountain."
            },
            {
                "heading": "YHWH Sings Over His People (3:14-20)",
                "body": "The most tender restoration passage in the prophets. Israel is called to sing, "
                        "shout, rejoice. YHWH has taken away their judgments. YHWH their God is in their "
                        "midst -- the mighty warrior who saves. He rejoices with gladness, quiets with love, "
                        "exults with singing. He will gather the outcasts, deal with oppressors, save the "
                        "lame, and change their shame into praise."
            }
        ]
    }
]
